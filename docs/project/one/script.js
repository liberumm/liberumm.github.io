const pivotTableApp = (() => {
    ...
    const DragDropHandler = {
        ...
        dragEnd(event) {
            ...
            if (!validDropTarget) {
                DragDropHandler.handleInvalidDrop();
            }
            initialParent = null;
            draggingItem = null;
        ...
        drop(event, area) {
            ...
            if (area === 'filterArea') {
                ...
                console.log("フィルターエリアにアイテムを追加しました:", data);
            } else if (area === 'rowArea' || area === 'columnArea' || area === 'valueArea') {
                // 行エリア、列エリア、集計値エリアへのドロップイン処理
                if (initialParentId !== 'columns' && initialParentId !== 'filterArea') {
                    initialParent.removeChild(draggingItem);
                }
                ...
                // 有効なドロップ先以外へのドロップ処理
                if (initialParentId === 'filterArea') {
                    Utils.removeItemFromArea('filterArea', data);
                    initialParent.removeChild(draggingItem);
                } else {
                    Utils.removeItemFromArea(initialParentId, data);
                    DragDropHandler.addItemToColumns(draggingItem.textContent);
                    initialParent.removeChild(draggingItem);
                }
                console.log("無効なドロップ先です:", data);
            }
