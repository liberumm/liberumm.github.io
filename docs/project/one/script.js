const pivotTableApp = (() => {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        ['rowArea', 'columnArea', 'valueArea'].forEach(area => {
            ...
            areaElement.addEventListener('drop', (event) => DragDropHandler.dropToOtherArea(event, area));
        ...
        filterAreaElement.addEventListener('dragover', DragDropHandler.dragOver);
        filterAreaElement.addEventListener('drop', DragDropHandler.dropToFilterArea);
        filterAreaElement.addEventListener('dragenter', DragDropHandler.dragEnter);
        filterAreaElement.addEventListener('dragleave', DragDropHandler.dragLeave);
    });
    ...
    const DragDropHandler = {
        ...
        dragEnd(event) {
            ...
            const validDropTarget = document.querySelector('.droppable-area.hovered');
            if (!validDropTarget) {
                if (initialParent.id !== 'columns' && initialParent.id !== 'filterArea') {
                    Utils.removeItemFromArea(initialParent.id, draggingItem.textContent);
                    DragDropHandler.addItemToColumns(draggingItem.textContent);
                    initialParent.removeChild(draggingItem);
                } else if (initialParent.id === 'filterArea') {
                    Utils.removeItemFromArea(initialParent.id, draggingItem.textContent);
                    initialParent.removeChild(draggingItem);
                }
                console.log("有効なドロップ先が見つかりませんでした。アイテムを元の位置に戻します。");
            }
            initialParent = null;
            draggingItem = null;
        ...
        dropToOtherArea(event, area) {
            ...
            Utils.removeFromOtherAreas(area, data);
            activeItems[area].push(data);
            FilterHandler.updateFilterControls();
            TableHandler.updatePivotTable();
            console.log("アイテムがエリアにドロップされました:", data, "エリア:", area);
        },

        dropToFilterArea(event) {
            event.preventDefault();
            event.target.classList.remove('hovered');
            const data = event.dataTransfer.getData('text/plain');
            const existingItems = Array.from(event.target.children).map(child => child.textContent);
            ...
            if (!activeItems.filterArea.includes(data)) {
                activeItems.filterArea.push(data);
            }
            console.log("アイテムがフィルターエリアにドロップされました:", data);
            FilterHandler.updateFilterControls();
            TableHandler.updatePivotTable();
        },
