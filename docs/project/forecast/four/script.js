...
function readFile(file, period) {
    Papa.parse(file, {
        ...
        complete: function(results) {
            ...
            if (debugMode) console.log(`Data for ${period}:`, data);  // データをコンソールに表示
            if (period === 'all') {
...
function normalizeData(data) {
    ...
    return data.map(d => {
        ...
        d['month'] = d['販売日時'].getMonth() + 1;
        d['day'] = d['販売日時'].getDate();
        d['weekday'] = d['販売日時'].getDay();
        return d;
    });
}
...
async function predictSales(period1, period2, period3, days, startDate) {
    ...
    const { model: trainedModel, lossValues: trainingLossValues } = await buildModel(inputData, outputData);
    model = trainedModel;
    lossValues = trainingLossValues;
    ...
    for (let i = 0; i < days; i++) {  // 修正: ループ開始を0からに変更して終了日も含める
        ...
        const trendAdjustment = getTrendAdjustment(period2, currentDate);
        predictions.push({ date: new Date(currentDate), prediction: prediction + trendAdjustment });
        currentDate.setDate(currentDate.getDate() + 1);
    }

    if (debugMode) console.log('Predictions with trend adjustment:', predictions);  // 予測結果をコンソールに表示
    plotForecast(dataPeriod1, dataPeriod2, dataPeriod3, predictions, startDate);
    plotLoss(lossValues);
    updateForecastTable(predictions, period2);
}

function getTrendAdjustment(period2, currentDate) {
    const lastYearDate = new Date(currentDate);
    lastYearDate.setFullYear(lastYearDate.getFullYear() - 1);

    const correspondingLastYearData = period2.find(d => d['販売日時'].getMonth() === lastYearDate.getMonth() &&
                                                       d['販売日時'].getDate() === lastYearDate.getDate());

    if (correspondingLastYearData) {
        return correspondingLastYearData['売上金額'];
    } else {
        return 0;
    }
}
...
async function buildModel(inputData, outputData) {
    ...
    model.add(tf.layers.dense({ units: 50, inputShape: [4], activation: 'relu' }));
    model.add(tf.layers.dense({ units: 25, activation: 'relu' }));
    model.add(tf.layers.dense({ units: 1 }));
    model.compile({ loss: 'meanSquaredError', optimizer: 'adam' });

    const xs = tf.tensor2d(inputData.map(d => [d['normalizedTimestamp'], d['month'], d['day'], d['weekday']]), [inputData.length, 4]);
    const ys = tf.tensor2d(outputData, [outputData.length, 1]);

    const lossValues = [];
    if (debugMode) console.log('Training model...');  // モデル訓練開始のログ
    ...
    await model.fit(xs, ys, {
        ...
        callbacks: {
            onEpochEnd: (epoch, logs) => {
                ...
                if (debugMode) console.log(`Epoch ${epoch + 1}: loss = ${logs.loss}`);  // 各エポックの損失値をログ表示
                updateLoadingOverlay(Math.round((epoch + 1) / 5) + '%');
            }
        }
    });

    if (debugMode) console.log('Model trained.');  // モデル訓練終了のログ
...
async function makePrediction(model, date, inputData) {
    ...
    const input = tf.tensor2d([[normalizedTimestamp, month, day, weekday]], [1, 4]);
    const output = model.predict(input);
    const prediction = output.dataSync()[0];
    if (debugMode) console.log(`Prediction for ${date}: ${prediction}`);  // 予測値をログ表示
    return prediction;
}
...
function plotForecast(period1, period2, period3, predictions, startDate) {
    ...
    if (debugMode) console.log('Plotting forecast...');  // プロット開始のログ
    ...
    if (debugMode) console.log('Forecast plotted.');  // プロット終了のログ
}
...
function plotLoss(lossValues) {
    ...
    if (lossValues.length === 0) {
        lossValues.push(0); // 損失値がない場合はゼロを設定
    }

    if (debugMode) console.log('Loss values:', lossValues);  // 損失値をコンソールに表示
...
function updateForecastTable(predictions, period2) {
    ...
    predictions.forEach(prediction => {
        ...
        const correspondingLastYearData = getCorrespondingLastYearData(period2, prediction.date);
        newRow.insertCell().textContent = correspondingLastYearData ? correspondingLastYearData['売上金額'].toFixed(2) : 'N/A';
    });
}

function getCorrespondingLastYearData(period2, currentDate) {
    const lastYearDate = new Date(currentDate);
    lastYearDate.setFullYear(lastYearDate.getFullYear() - 1);

    return period2.find(d => d['販売日時'].getFullYear() === lastYearDate.getFullYear() &&
                             d['販売日時'].getMonth() === lastYearDate.getMonth() &&
                             d['販売日時'].getDate() === lastYearDate.getDate());
}
