const pivotTableApp = (() => {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        ['rowArea', 'columnArea', 'valueArea', 'filterArea', 'trashArea', 'columns'].forEach(area => {
    ...
    const DragDropHandler = {
        ...
        drop(event, area) {
            ...
            if (area === 'trashArea') {
                Utils.removeItemFromAllAreas(data);
                return;
            }
                    ...
                    <div class="accordion" id="accordion-${field}">
                        <div class="card">
                            <div class="card-header" id="heading-${field}">
                                <h5 class="mb-0">
                                    <button class="btn btn-link" type="button" data-toggle="collapse" data-target="#collapse-${field}" aria-expanded="true" aria-controls="collapse-${field}">
                                        ${field}
                                    </button>
                                </h5>
                            </div>
                            <div id="collapse-${field}" class="collapse" aria-labelledby="heading-${field}" data-parent="#accordion-${field}">
                                <div class="card-body">
                                    ...
                                    <button class="btn btn-secondary btn-sm mt-2" onclick="pivotTableApp.FilterHandler.toggleCheckboxes('${field}', true)">全選択</button>
                                    <button class="btn btn-secondary btn-sm mt-2" onclick="pivotTableApp.FilterHandler.toggleCheckboxes('${field}', false)">全解除</button>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    const Utils = {
        ...
        removeItemFromAllAreas(text) {
            ['rowArea', 'columnArea', 'valueArea', 'filterArea'].forEach(area => {
                Utils.removeItemFromArea(area, text);
            });
            console.log("アイテムがすべてのエリアから削除されました:", text);
        }
    };
