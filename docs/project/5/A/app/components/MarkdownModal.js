(function(){
  const { Table, TableHead, TableRow, TableCell, TableBody, Checkbox, TextField, Button, ToggleButton, ToggleButtonGroup, Dialog, DialogTitle, DialogContent, DialogActions, IconButton, Grid, Card, CardContent } = MaterialUI;
  ...
  window.MarkdownModal = function MarkdownModal(props){
    ...
    const addToAction = (product)=>{
      setActionForm(prev=>({ ...prev, rows: {...prev.rows, [product.sku]: {...prev.rows?.[product.sku], qty:'', discountAmount:'', newPrice:'', toStore:'', confirmer:''}} }));
  ...
  window.MarkdownModalDialog = function MarkdownModalDialog(props){
    return <window.MarkdownModal {...props} isModal={true} />;
  };
})();
