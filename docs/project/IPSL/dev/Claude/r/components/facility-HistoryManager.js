...
function HistoryManager() {
  ...
  const paginatedHistories =
    rowsPerPage > 0 ? filteredHistories.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage) : filteredHistories;
  ...
  const handleSaveHistory = () => {
    ...
    if (dialogMode === "new") {
      const newId = histories.length > 0 ? Math.max(...histories.map((h) => h.history_id)) + 1 : 1;
      const newRecord = {
        ...currentHistory,
        history_id: newId,
        created_at: now,
      ...
      const updatedHistories = histories.map((h) =>
        h.history_id === currentHistory.history_id ? { ...currentHistory, updated_at: now } : h
      );
      ...
      <Box mb={2}>
        ...
        >
          {["全て", "complaint", "repair", "meeting", "approval", "donation"].map((cat) => (
            <Tab key={cat} value={cat} label={cat === "全て" ? "全て" : categoryMapping[cat]} />
          ))}
        </Tabs>
      </Box>
          ...
          <TableBody>
            {paginatedHistories.map((h) => (
              <TableRow key={h.history_id}>
                ...
                <TableCell sx={{ whiteSpace: 'nowrap' }}>{categoryMapping[h.category]}</TableCell>
                <TableCell sx={{ whiteSpace: 'nowrap' }}>{h.title}</TableCell>
                <TableCell sx={{ whiteSpace: 'nowrap' }}>{h.event_date}</TableCell>
                <TableCell sx={{ whiteSpace: 'nowrap' }}>{statusMapping[h.status]}</TableCell>
                ...
                <TableCell sx={{ whiteSpace: 'nowrap' }}>
                  <Button size="small" onClick={() => handleOpenEdit(h)}>
                    編集
                  </Button>
                  <Button size="small" color="secondary" onClick={() => handleDelete(h.history_id)}>
                    削除
                  </Button>
                </TableCell>
              </TableRow>
