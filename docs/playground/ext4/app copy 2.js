...
function exportTableToExcel() {
    ...
    // カスタムヘッダーを作成
    const customHeaders = ["", "No", "納品日", "商品コード", "商品名", "原価", "売価", "係数パターン", "配分数", "単位", "最低導入数", "一括数量", "導入店舗数", "合計", "原価計", "売価計", "値入率"];
    ...
    for (let row of rows) {
        ...
        if (minimumAllocation > 0) {
            ...
            if (minimumTotal <= totalQuantity) {
                ...
                //console.error("Total allocation is less than the minimum required allocation.");
                //alert("Total allocation is less than the minimum required allocation.");
                continue;
            }
        }
        ...
        console.log("totalQuantityAllocationx:" + totalQuantityAllocation);
        console.log("remainder:" + remainder);
...
function parseProductList(workbook) {
    const sheet = workbook.Sheets[workbook.SheetNames[0]];
    const data = XLSX.utils.sheet_to_json(sheet, { header: 1 });
    productList = data.slice(1); // 1行目をヘッダーとして2行目からデータを取り込む
...
function fillProductList() {
    ...
    productList.forEach((product, index) => {
        ...
        // 商品リストのデータを各セルに設定
        row.cells[2].querySelector('input').value = product[0]; // 納品日
        row.cells[3].querySelector('input').value = product[1]; // 商品コード
        row.cells[4].querySelector('input').value = product[2]; // 商品名
        row.cells[5].querySelector('input').value = product[3]; // 原価
        row.cells[6].querySelector('input').value = product[4]; // 売価
        row.cells[7].querySelector('input').value = product[5]; // 係数パターン
        row.cells[8].querySelector('input').value = product[6]; // 配分数
        row.cells[9].querySelector('input').value = product[7]; // 単位
        row.cells[10].querySelector('input').value = product[8]; // 最低導入数
    });
}
...
function updateCoefficientPatternOptions() {
    ...
    rows.forEach(row => {
        const select = row.querySelector('td:nth-child(7) input');
        if (select) {
            // 既存の選択肢をクリア
            select.innerHTML = '';
